...
def GUI():
    ...
    frame_input = Frame(frame_text)
    frame_result = Frame(frame_text)
    frame_text.pack(side=LEFT)

    lable_input = Label(master=frame_input, text="Input:")
    lable_input.pack(anchor='w')
    text_input = Text(frame_input, height=17, width=60,
                      highlightbackground='#000', highlightthickness=1)
    text_input.pack(pady=5)
    frame_input.pack(padx=10, pady=10)

    lable_result = Label(master=frame_result, text="Result:")
    lable_result.pack(anchor='w')
    text_result = Text(frame_result, height=17, width=60,
                       highlightbackground='#000', highlightthickness=1)
    text_result.pack()
    frame_result.pack(padx=10, pady=5)
    ...
    frame_btn.pack(side=LEFT, padx=20, pady=30, anchor='n')
    root.mainloop()
